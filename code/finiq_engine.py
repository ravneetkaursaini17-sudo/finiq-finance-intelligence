...
APPLE_SAMPLE_CSV = "apple_financials.csv"  # you’ll create this from Apple 10-K
...
def load_financial_csv(file_path: str) -> pd.DataFrame:
    """
    Loads a user-uploaded CSV and validates structure.
    """
...
def plot_financial_trends(model: FinancialModel):
    ...
    ax1.yaxis.set_major_formatter(FuncFormatter(lambda x, pos: f"${x/1_000_000:.0f}M"))
...
def load_apple_sample(csv_path: str = APPLE_SAMPLE_CSV) -> pd.DataFrame:
